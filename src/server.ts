...
import { schemas, CONSTANTS, Metrics, ServerConfig, SchemaInput } from './types.js';
...
function createMCPServer(): McpServer {
      ...
      try {
        ...
        switch (op) {
          ...
          case 'divide':
            ...
            if (b === 0) {
              requestLogger.error('Division by zero attempted, returning InvalidParams error', { a, b });
  ...
  server.prompt(
    ...
    async ({ calculation, level = 'intermediate' }: SchemaInput<'explainCalculation'>): Promise<GetPromptResult> => {
...
Make the explanation educational and easy to follow.`,
  ...
  server.prompt(
    ...
    async ({ topic, difficulty = 'medium', count = '5' }: SchemaInput<'generateProblems'>): Promise<GetPromptResult> => {
...
2. [Answer with reasoning and method]`,
  ...
  server.prompt(
    ...
    async ({ topic, studentLevel = 'intermediate' }: SchemaInput<'calculatorTutor'>): Promise<GetPromptResult> => {
      ...
      const levelGuidance = {
        beginner: 'Use very simple language, concrete examples, and break down concepts into tiny steps',
        intermediate: 'Use clear explanations with some mathematical terminology and visual examples',
        advanced: 'Engage with complex concepts, encourage critical thinking, and explore connections'
      };
...
async function startServer(): Promise<void> {
  try {
     ...
     */
    process.on('SIGTERM', shutdown); // Kubernetes, Docker, systemd
